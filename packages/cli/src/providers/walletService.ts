...
export class WalletService {
  ...
  loadWallet(i?:string): Wallet | null {
    const dataDir = this.configService.getDataDir();
    let walletFile = join(dataDir, `wallet_${i}.json`);
    if (!!i) {
      walletFile = join(dataDir, `wallet_${i}.json`);
    } else {
  ...
  getAddress(i?:string): btc.Address {
    
    console.log("print params i: ", i)
